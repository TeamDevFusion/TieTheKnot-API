...
export class Initial1730232088365 implements MigrationInterface {
    name = 'Initial1730232088365'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`user_logs\` (\`id\` varchar(36) NOT NULL, \`ip\` varchar(255) NULL, \`url\` varchar(255) NULL, \`action\` varchar(255) NOT NULL, \`snapshot\` json NULL, \`endpoint\` varchar(255) NOT NULL, \`method\` enum ('GET', 'POST', 'PATCH', 'PUT', 'DELETE') NULL, \`exception\` json NULL, \`sessionId\` varchar(255) NULL, \`byId\` varchar(255) NULL, \`createdAt\` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP, \`userId\` varchar(255) NULL, \`vendorTypeId\` varchar(255) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`vendor_types\` (\`id\` varchar(36) NOT NULL, \`createdAt\` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updatedAt\` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP, \`deletedAt\` timestamp(6) NULL, \`name\` varchar(255) NOT NULL, \`label\` varchar(255) NULL, \`icon\` varchar(255) NULL, \`status\` tinyint NOT NULL DEFAULT 1, \`createdById\` varchar(36) NULL, \`updatedById\` varchar(36) NULL, \`deletedById\` varchar(36) NULL, UNIQUE INDEX \`UNIQUE_vendorType_name\` (\`name\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`users\` ADD CONSTRAINT \`FK_user_vendorType\` FOREIGN KEY (\`vendorTypeId\`) REFERENCES \`vendor_types\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`users\` DROP FOREIGN KEY \`FK_user_vendorType\``);
        ...
        await queryRunner.query(`DROP INDEX \`UNIQUE_vendorType_name\` ON \`vendor_types\``);
